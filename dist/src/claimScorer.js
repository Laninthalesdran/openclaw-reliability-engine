...
export function scoreClaim(claim, sources) {
    ...
    for (const k of Object.keys(W))
        raw += W[k] * dims[k];
    const vetoes = [];
    if (dims.falsifiability < 0.15) {
        raw = Math.min(raw, 0.20);
        vetoes.push("unfalsifiable");
    }
    if ((dims.provenance + dims.verifiability) / 2 < 0.15) {
        raw = Math.min(raw, 0.25);
        vetoes.push("no checkable origin");
    }
...
export function explain(text, r) {
    const b = r.breakdown;
    const lines = [
        `CLAIM: ${text}`,
